...
def test_enterprise_admin_entry_is_optional(app_server) -> None:
    """Enterprise admin entry point should return 404 when no build exists."""
    # 企业工作台静态目录默认不存在，应返回 404 而非崩溃
    response = app_server.api_request("GET", "/enterprise-admin/")
    # 可能是 404（无构建）或 200（有构建），两者都是可接受的
    assert response.status_code in (200, 404), app_server.logs_tail()
