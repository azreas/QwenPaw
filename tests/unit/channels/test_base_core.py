...
@pytest.mark.asyncio
async def test_send_event_sends_completed_response_output(base_channel):
    """send_event should deliver AgentResponse output messages to channels."""
    from agentscope_runtime.engine.schemas.agent_schemas import (
        AgentResponse,
        ContentType,
        Message,
    ...
    message = Message(
        ...
        status=RunStatus.Completed,
        content=[
            TextContent(type=ContentType.TEXT, text="租户回复"),
        ],
    )
    ...
    base_channel.send_message_content.assert_awaited_once_with(
        "alice",
        message,
        {"wecom_frame": {}},
    )
...
@pytest.mark.asyncio
async def test_run_process_loop_deduplicates_response_output(base_channel):
    """Response output must not resend a message already emitted."""
    from agentscope_runtime.engine.schemas.agent_schemas import (
        AgentResponse,
        ContentType,
        Message,
    ...
    message = Message(
        ...
        role=Role.ASSISTANT,
        status=RunStatus.Completed,
        content=[TextContent(type=ContentType.TEXT, text="只回一次")],
    )
    response = AgentResponse(
        status=RunStatus.Completed,
        output=[message],
...
@pytest.mark.asyncio
async def test_send_event_deduplicates_replayed_response_output(base_channel):
    """send_event must not resend the same completed response.output twice."""
    from agentscope_runtime.engine.schemas.agent_schemas import (
        AgentResponse,
        ContentType,
        Message,
    ...
    message = Message(
        ...
        role=Role.ASSISTANT,
        status=RunStatus.Completed,
        content=[TextContent(type=ContentType.TEXT, text="只发一次")],
    )
    response = AgentResponse(
        id="resp-dup-1",
        status=RunStatus.Completed,
