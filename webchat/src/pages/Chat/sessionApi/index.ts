 ...
 */
const toOutputMessage = (msg: any): OutputMessage => ({
  ...msg,
  role:
...
/**
 * Build an assistant responses card (AgentScopeRuntimeResponseCard)
...
const isWecomSingleSessionId = (sessionId: string | undefined): boolean =>
  Boolean(sessionId?.startsWith("wecom:")) &&
  !sessionId?.startsWith("wecom:group:");

const legacyLocalSessionId = (sessionId: string): string | null => {
  if (!sessionId.startsWith("webchat:")) return null;
  const parts = sessionId.split(":");
  return parts.length >= 3 ? parts[parts.length - 1] : null;
};

const matchesSessionIdentifier = (
  session: IAgentScopeRuntimeWebUISession,
  identifier: string | null | undefined,
): boolean => {
  if (!identifier) return false;
  const ext = session as ExtendedSession;
  const sessionId = ext.sessionId || "";
  return (
    session.id === identifier ||
    ext.realId === identifier ||
    sessionId === identifier ||
    legacyLocalSessionId(sessionId) === identifier
  );
};

const sessionChannel = (session: IAgentScopeRuntimeWebUISession): string =>
  (session as ExtendedSession).channel || DEFAULT_CHANNEL;

const sameSessionSource = (
  a: IAgentScopeRuntimeWebUISession,
  b: IAgentScopeRuntimeWebUISession,
): boolean => sessionChannel(a) === sessionChannel(b);

const sameSessionRecord = (
  existing: IAgentScopeRuntimeWebUISession,
  next: IAgentScopeRuntimeWebUISession,
): boolean => {
  if (!sameSessionSource(existing, next)) return false;
  const existingExt = existing as ExtendedSession;
  const nextExt = next as ExtendedSession;
  return (
    existing.id === next.id ||
    existingExt.realId === next.id ||
    existing.id === nextExt.realId ||
    (
      Boolean(existingExt.sessionId) &&
      existingExt.sessionId === nextExt.sessionId
    )
  );
};

const chatSpecToSession = (chat: ChatSpec): ExtendedSession => {
  const rawSessionId = chat.session_id || chat.id;
  return {
    id: chat.id,
    ...
    messages: [],
    meta: chat.meta || {},
    generating: chat.status === "running",
...
const isGenerating = (chatHistory: any): boolean => {
...
const resolveRealId = (
  ...
  const realSession = sessionList.find(
    (s) => matchesSessionIdentifier(s, tempSessionId),
  ) || sessionList.find(
    (s) => isWecomSingleSessionId((s as ExtendedSession).sessionId),
  );
...
class SessionApi implements IAgentScopeRuntimeWebUISessionAPI {
  ...
  getRealIdForSession(sessionId: string): string | null {
    const s = this.sessionList.find((x) => matchesSessionIdentifier(x, sessionId)) as
      | ExtendedSession
      | undefined;
  ...
  async getSessionList() {
    ...
    this.sessionListRequest = (async () => {
      try {
        const response = await apiRequest<{ sessions: any[] }>("/webchat/sessions");
        const chats = response.sessions || [];
        
        const newList = (chats as ChatSpec[])
          .filter((c) => c.id && c.id !== "undefined" && c.id !== "null")
          .map(chatSpecToSession) as IAgentScopeRuntimeWebUISession[];

        // Merge with existing sessionList to preserve local state without
        // crossing WebChat and WeCom Bot records that share a session_id.
        this.sessionList = newList.map((s) => {
          const existing = this.sessionList.find(
            (e) => sameSessionRecord(e, s),
          ) as ExtendedSession | undefined;
          if (!existing) return s;
          const next = { ...s } as ExtendedSession;
          // 如果已有 realId 映射，保留它（表示这是一个从临时 ID 解析来的会话）
          if (existing.realId) {
            next.id = existing.id;  // 保留临时 ID
            next.realId = existing.realId;  // 保留真实 UUID
          }
          if (existing.generating !== undefined) {
            next.generating = existing.generating;
          }
          if (existing.name && existing.name !== DEFAULT_SESSION_NAME) {
            next.name = existing.name;  // 保留已更新的名称
          }
          return next as IAgentScopeRuntimeWebUISession;
        });

        // If there's a preferred chat ID, move it to the front
        if (this.preferredChatId) {
          const preferredId = this.preferredChatId;
          this.preferredChatId = null;
          // 匹配 id 或 realId（id 可能是临时 ID，realId 是 UUID）
          const idx = this.sessionList.findIndex((s) =>
            matchesSessionIdentifier(s, preferredId),
          );
          if (idx > 0) {
  ...
  private async _doGetSession(
    ...
    if (isLocalTimestamp(sessionId)) {
      ...
      if (fromList?.realId) {
        const chatHistory = await apiRequest<any>(`/webchat/sessions/${fromList.realId}`);
      ...
      const refreshed = this.sessionList.find((s) => s.id === sessionId) as
        ...
        | undefined;
      if (refreshed?.realId) {
        const chatHistory = await apiRequest<any>(`/webchat/sessions/${refreshed.realId}`);
        const generating = isGenerating(chatHistory);
        const messages = convertMessages(chatHistory.messages || []);
    ...
    const fromList = this.sessionList.find((s) =>
      matchesSessionIdentifier(s, sessionId),
    ) as
      | ExtendedSession
      | undefined;

    const fetchSessionId = fromList?.realId ?? fromList?.id ?? sessionId;
    const chatHistory = await apiRequest<any>(`/webchat/sessions/${fetchSessionId}`);
    const generating = isGenerating(chatHistory);
    const messages = convertMessages(chatHistory.messages || []);
    this.patchLastUserMessage(messages, generating, fetchSessionId);
    const session: ExtendedSession = {
      id: fromList?.id || sessionId,
      name: fromList?.name || chatHistory.name || sessionId,
      sessionId: fromList?.sessionId || sessionId,
      userId: fromList?.userId || DEFAULT_USER_ID,
      channel: fromList?.channel || DEFAULT_CHANNEL,
      messages,
  ...
  async createSession(_session?: Partial<IAgentScopeRuntimeWebUISession>) {
    ...
    const refreshed = await this.getSessionList();
    const createdSession = refreshed.find((s) =>
      matchesSessionIdentifier(s, createdSessionId),
    ) as ExtendedSession | undefined;
    ...
    if (!createdSession) {
      this.sessionList = [effectiveSession, ...this.sessionList];
    }
    if (_session) {
    ...
    this.updateWindowVariables(effectiveSession);
    return [...this.sessionList];
  }
  ...
  async removeSession(session: Partial<IAgentScopeRuntimeWebUISession>) {
    ...
    if (deleteId) {
      try {
        await apiRequest(`/webchat/sessions/${deleteId}`, {
          method: "DELETE",
        });
      } catch (error) {
