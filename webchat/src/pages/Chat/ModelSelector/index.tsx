...
import { webchatRoutePrefix } from "../../../utils/deployment";
...
export default function ModelSelector() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // First get the current user's agent
      const userData = await apiRequest<{ agent_id: string; user_id: string; username: string }>("/webchat/me");
      const currentAgentId = userData.agent_id;
  ...
  useEffect(() => {
    ...
    const comingToChat =
      curr.startsWith(`${webchatRoutePrefix}/chat`) &&
      !prev.startsWith(`${webchatRoutePrefix}/chat`);
    if (comingToChat && agentId) {
      providerApi
        .getActiveModels({
          scope: "effective",
          agent_id: agentId,
  ...
  return (
    ...
    >
      <Tooltip title={t("chat.modelSelectTooltip")} mouseEnterDelay={0.5} placement="left">
        <div
