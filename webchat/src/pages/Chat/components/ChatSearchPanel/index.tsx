  ...
  const handleResultClick = useCallback(
    (result: SearchResult) => {
      const session = sessions.find((s) => {
        const extSession = s as any;
        const realId = sessionApi.getRealIdForSession(s.id || "");
        return (
          s.id === result.chatId ||
          extSession.realId === result.chatId ||
          realId === result.chatId ||
          extSession.sessionId === result.chatId ||
          legacyLocalSessionId(extSession.sessionId) === result.chatId
        );
      });

      if (session?.id) {
        setCurrentSessionId(session.id);
        navigate(webchatPath(`/chat/${session.id}`));
      } else {
        navigate(webchatPath(`/chat/${result.chatId}`));
      }
