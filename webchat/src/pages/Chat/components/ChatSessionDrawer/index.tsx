...
const ChatSessionDrawer: React.FC<ChatSessionDrawerProps> = (props) => {
  ...
  const handleCreateSession = useCallback(async () => {
    // 使用 AgentScope 原生 createSession，与 console 保持一致
    await createSession();
    props.onClose();
  ...
  const refreshSessions = useCallback(async () => {
    const list = await sessionApi.getSessionList() as unknown as IAgentScopeRuntimeWebUISession[];
    setSessions(list);
  }, [setSessions]);
        ...
        <div className={styles.list}>
          {sortedSessions.map((session) => {
            ...
            return (
              <ChatSessionItem
                key={session.id}
                name={typeof session.name === 'string' ? session.name : (session.name ? String(session.name) : "New Chat")}
                ...
                onEdit={() =>
                  handleEditStart(session.id!, typeof session.name === 'string' ? session.name : (session.name ? String(session.name) : "New Chat"))
                }
