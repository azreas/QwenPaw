...
async def get_agent_for_request(
    ...
    # Load config only for active-agent fallback.
    if not target_agent_id:
    ...
    try:
        resolved_ref = resolver.resolve(target_agent_id)
    except (json.JSONDecodeError, ValidationError, ValueError) as e:
        raise HTTPException(
            status_code=422,
            detail=f"Agent '{target_agent_id}' configuration is invalid: {e}",
        ) from e
    ...
    try:
        workspace = await manager.get_agent(resolved_ref.agent_id)
        if not workspace:
            raise HTTPException(
                status_code=404,
                detail=f"Agent '{target_agent_id}' not found",
