...
if TYPE_CHECKING:
    ...
    from ...enterprise.interfaces import RuntimeExtensionBundle

logger = logging.getLogger(__name__)


async def _resolve_runtime_extensions(
    runtime: Any | None,
    ctx: "RequestContext",
) -> "RuntimeExtensionBundle":
    """解析运行时扩展。runtime 为 None 时返回空 bundle。"""
    start = time.monotonic()
    from ...enterprise.interfaces import (
        RuntimeExtensionBundle,
    )
    ...
    if extensions is None:
        return RuntimeExtensionBundle()
    try:
        bundle = await extensions.resolve(ctx)
        await _emit_runtime_extension_trace(
            runtime=runtime,
            ctx=ctx,
            status="success",
        ...
        logger.exception(
            "Runtime extension resolution failed"
        )
        await _emit_runtime_extension_trace(
            runtime=runtime,
            ctx=ctx,
            status="degraded",
            duration_ms=(time.monotonic() - start) * 1000,
            error_code="runtime_extension.resolve_failed",
            error_reason=str(exc)[:200],
        )
        return RuntimeExtensionBundle()


async def _emit_runtime_extension_trace(
    *,
    runtime: Any,
    ctx: "RequestContext",
    status: str,
    ...
    if audit_bus is None:
        return
    try:
        from ...enterprise.audit.emit import emit_platform_invocation_event
        ...
        request = type(
            ...
            (),
            {
                ...
                "state": type(
                    "TraceState",
                    (),
                    {"request_context": ctx, "request_id": "", "trace_id": ""},
                )(),
            },
        )()
        await emit_platform_invocation_event(
            request=request,
            call_type="runtime_extension",
            call_name="runtime_extension.resolve",
            status=status,
        ...
        logger.debug("runtime extension trace emission failed", exc_info=True)
...
class AgentRunner(Runner):
    def __init__(
        ...
        self._preloaded_agent_config = agent_config
        self._enterprise_runtime = None  # 企业运行时扩展，由 Workspace 注入
    ...
    async def _emit_trace(
        ...
        *,
        ...
        enterprise_runtime 或 audit bus 不可用时静默跳过。

        request_ctx 可选：当不在 enterprise_context_scope 内时
        （如异常处理），显式传入 RequestContext 以避免丢失租户/入口字段。

        extra_payload 可选：追加到 payload 的额外字段，
        如 MCP 调用的 mcp_name。
        """
        if self._enterprise_runtime is None:
            return
        audit_bus = getattr(self._enterprise_runtime, "audit", None)
        if audit_bus is None:
            return
        try:
            from ...enterprise.audit.emit import emit_platform_invocation_event
            from ...enterprise.context import get_current_request_context

            ctx = request_ctx or get_current_request_context()
            call_type = call_type or ability_type
            call_name = call_name or ability_name
            request = type(
                "TraceRequest",
                (),
                {
                    "app": type(
                        ...
                        (),
                        {
                            "state": type(
                                "TraceAppState",
                                (),
                                {"enterprise_runtime": self._enterprise_runtime},
                            )(),
                        },
                    )(),
                    "state": type(
            ...
            await emit_platform_invocation_event(
                ...
                resource_id=call_name,
                metadata=extra_payload,
                legacy_business_event=call_type in {"skill", "mcp"},
            )
        except Exception:
            logger.debug(
                "emit_trace failed for %s:%s",
                call_type or ability_type,
                call_name or ability_name,
                exc_info=True,
            )
    ...
    @staticmethod
    def _find_skill_by_command_name(
        name: str,
        skills: dict,
    ) -> dict | None:
        """Find a skill by its workspace folder name."""
        return next(
            (
                s
    ...
    @staticmethod
    def _resolve_existing_skill_command(
        query: str | None,
        skills: dict,
    ) -> tuple[str, bool] | None:
        """Resolve query to an installed skill command.

        Returns ``(skill_name, has_user_input)`` only when the command
        actually maps to an installed skill. Unknown slash commands must not
        trigger skill RBAC checks or audit traces.
        """
        if not query:
            return None
        ...
        if AgentRunner._find_skill_by_command_name(name, skills) is None:
            return None
        return name, bool(user_input)
    ...
    def _maybe_inject_skill(
        ...
        *skills* is ``agent.toolkit.skills`` — already resolved for
        ...
        skill = AgentRunner._find_skill_by_command_name(name, skills)
        if not skill:
            return None
    ...
    async def query_handler(
        ...
        **kwargs,
        ...
        _skill_name_for_trace = ""
        _skill_has_input = False
        mcp_clients: list = []
        try:
            ...
            agent_config = self._preloaded_agent_config or load_agent_config(self.agent_id)
            ...
            from ...enterprise.context_builders import (
            ...
            # 安全访问 request.state — AgentRequest 可能没有 state
            ...
            # P3-2: 从 channel_meta 提取上游传入的 roles，
            # 避免 wx_* agent 默认升级为 tenant_member。
            # 只要 meta 中有 roles 键（包括空列表）就传递，
            # 显式空 roles 不应被提升为 tenant_member。
            ...
            request_context = build_context_from_runner_payload(
                request_id=_req_id,
                trace_id=_trace_id,
            ...
            if request_context.root_session_id != session_id:
                set_current_root_session_id(request_context.root_session_id)
                root_preview = (
                    request_context.root_session_id[:12]
                    if len(request_context.root_session_id) >= 12
                    else request_context.root_session_id
                )
                logger.debug(
            ...
            base_request_context = {
                ...
                "root_session_id": request_context.root_session_id,
            ...
            # 解析运行时扩展（在获取 MCP clients 之前）
            extension_bundle = await _resolve_runtime_extensions(
                self._enterprise_runtime,
                request_context,
            )

            # P3-2: 检查 mcp:call 权限
            mcp_call_allowed = True
            if self._enterprise_runtime is not None:
                authz = getattr(self._enterprise_runtime, "authz", None)
                if authz is not None:
                    decision = await authz.check_permission(
                        request_context, "mcp", "call",
                    )
                    mcp_call_allowed = decision.allowed

            # Get MCP clients from manager (hot-reloadable)
            # mcp_clients 已在 try 顶部初始化为 []
            if self._mcp_manager is not None and mcp_call_allowed:
                mcp_clients = await self._mcp_manager.get_clients()

            # 合并 MCP clients
            if mcp_call_allowed:
                mcp_clients = mcp_clients + extension_bundle.mcp_clients
            else:
                logger.debug(
                    "MCP call permission denied for user=%s tenant=%s",
            ...
            # Load agent-specific configuration
            agent_config = (
                self._preloaded_agent_config
                or load_agent_config(self.agent_id)
            )

            logger.debug("Enabled MCP: %s", mcp_clients)
            ...
            agent = QwenPawAgent(
                ...
                tool_policy_patch=extension_bundle.tool_policy,
            )
            from ...enterprise.context_scope import enterprise_context_scope

            with enterprise_context_scope(
                request_context,
                extension_bundle.tool_policy,
            ):
                ...
                # P3-2: _skill_name_for_trace / _skill_has_input 已在 try 顶部初始化
                _skill_start = 0.0

                # Skill info (/<name> without input) is display-only
                if mission_info is None:
                    # P3-2: 只有确实匹配已安装 Skill 的命令才做权限检查和追踪。
                    # 未知 slash 命令应继续走原有对话路径，不记录虚假的 skill.call。
                    _existing_skill = AgentRunner._resolve_existing_skill_command(
                        query,
                        agent.toolkit.skills,
                    )
                    if _existing_skill is not None:
                        _skill_name_for_trace = _existing_skill[0]
                        _skill_has_input = _existing_skill[1]

                    # P3-2: 技能调用前检查 skills:call 权限
                    _skill_call_allowed = True
                    if _skill_has_input and self._enterprise_runtime is not None:
                        authz = getattr(self._enterprise_runtime, "authz", None)
                        if authz is not None:
                            decision = await authz.check_permission(
                                request_context, "skills", "call",
                            )
                            _skill_call_allowed = decision.allowed
                            if not _skill_call_allowed:
                                logger.info(
                                    "Skill call denied: user=%s skill=%s",
                                    request_context.user_id,
                                    _skill_name_for_trace,
                                )
                    ...
                    _skill_duration_ms = (
                        time.monotonic() - _skill_start
                    ) * 1000

                    if skill_response is not None:
                        # 仅展示信息（无用户输入），不需要 call 权限
                        yield skill_response, True
                        return

                    if _skill_has_input and _skill_name_for_trace:
                        if not _skill_call_allowed:
                            # P3-2: 追踪权限拒绝
                            await self._emit_trace(
                                ability_type="skill",
                                ability_name=_skill_name_for_trace,
                                status="failure",
                                duration_ms=_skill_duration_ms,
                                error_reason="permission_denied:skills:call",
                            )
                            yield Msg(
                                name=self.agent_name,
                                role="assistant",
                            ...
                            return
                        # P3-2: 技能上下文已注入，等待 agent(msgs) 实际执行后
                        # 在下方统一记录成功/失败/耗时追踪
                ...
                if mission_info is not None:
                    ...
                    # MCP 追踪：从 toolkit middleware 收集的真实 tool 调用记录
                    _mcp_traces = getattr(agent, "_mcp_tool_traces", [])
                    for _trace in _mcp_traces:
                        await self._emit_trace(
                            ability_type="mcp",
                            ability_name=f"{_trace.mcp_name}/{_trace.tool_name}",
                            status=_trace.status,
                            duration_ms=_trace.duration_ms,
                            error_reason=_trace.error_reason,
                            extra_payload={"mcp_name": _trace.mcp_name},
                        )
                    _builtin_traces = getattr(agent, "_builtin_tool_traces", [])
                    for _trace in _builtin_traces:
                        await self._emit_trace(
                            call_type="builtin_tool",
                            call_name=_trace.tool_name,
                            status=_trace.status,
                            duration_ms=_trace.duration_ms,
                            error_code=(
                                "builtin_tool.failed"
                                if _trace.status != "success"
                                else ""
                            ),
                            error_reason=_trace.error_reason,
                        )
            ...
            # P3-2: agent 执行失败时的业务调用追踪
            _err_ctx = locals().get("request_context")
            _err_reason = str(e)[:200]
            await self._emit_trace(
                call_type="agent",
                call_name="agent.run",
                status="failure",
                ...
                request_ctx=_err_ctx,
            )
            if _skill_name_for_trace:
                await self._emit_trace(
                    ability_type="skill",
                    ability_name=_skill_name_for_trace,
                    status="failure",
            ...
            # MCP 追踪：从 toolkit middleware 收集的真实 tool 调用记录
            # middleware 在异常前已完成的部分记录仍需发送
            _mcp_traces = getattr(agent, "_mcp_tool_traces", []) if agent else []
            for _trace in _mcp_traces:
                await self._emit_trace(
                    ability_type="mcp",
                    ability_name=f"{_trace.mcp_name}/{_trace.tool_name}",
            ...
            _builtin_traces = getattr(agent, "_builtin_tool_traces", []) if agent else []
            for _trace in _builtin_traces:
                await self._emit_trace(
                    call_type="builtin_tool",
                    call_name=_trace.tool_name,
                    ...
                    duration_ms=_trace.duration_ms,
                    error_code=(
                        "builtin_tool.failed" if _trace.status != "success" else ""
                    ),
                    error_reason=_trace.error_reason,
                    request_ctx=_err_ctx,
                )
