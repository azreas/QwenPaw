...
class BaseChannel(ABC):
    ...
    def __init__(
        ...
        self._sent_event_message_ids: Dict[str, set[str]] = {}
            ...
            async for event in process_iterator:
                ...
                if obj == "message" and status == RunStatus.Completed:
                    if not handled_by_streaming:
                        ...
                        if event_id:
                            sent_message_ids.add(str(event_id))
                    ...
                    await self._send_response_output_messages(
                        request,
                        to_handle,
                        event,
                        send_meta,
    ...
    async def _send_response_output_messages(
        self,
        ...
        """Send completed response.output messages not already emitted."""
        if getattr(response, "status", None) != RunStatus.Completed:
            return
        for message in getattr(response, "output", None) or []:
            if getattr(message, "object", None) != "message":
                continue
            message_id = getattr(message, "id", None)
            if message_id and str(message_id) in sent_message_ids:
                continue
            if message_id:
                sent_message_ids.add(str(message_id))
            await self.on_event_message_completed(
                request,
                to_handle,
                message,
    ...
    @staticmethod
    def _delivery_dedup_key(*, user_id: str, session_id: str) -> str:
    ...
    async def send_event(
        ...
        *,
        ...
        Send completed message events directly, and completed response
        events by replaying their output messages.
        """
        obj = getattr(event, "object", None)
        status = getattr(event, "status", None)
        ...
        dedup_key = self._delivery_dedup_key(
            user_id=user_id,
            session_id=session_id,
        )
        sent_message_ids = self._sent_event_message_ids.setdefault(
            dedup_key,
            set(),
        )
        if obj == "message":
            event_id = getattr(event, "id", None)
            if event_id and str(event_id) in sent_message_ids:
                return
            if event_id:
                sent_message_ids.add(str(event_id))
