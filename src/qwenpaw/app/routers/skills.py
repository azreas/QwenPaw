...
class InstallUploadedSkillRequest(BaseModel):
...
def _build_workspace_skill_specs(workspace_dir: Path) -> list[SkillSpec]:
    ...
    for skill_name, raw_entry in sorted(entries.items()):
        ...
        try:
            ...
            installed_names.add(str(dump.get("name") or skill_name))
            specs.append(
                SkillSpec(
                    **dump,
    ...
    for name, skill_dir, skill in _uploaded_skill_candidates(workspace_dir):
...
def _uploaded_skill_candidates(workspace_dir: Path) -> list[tuple[str, Path, SkillInfo]]:
    media_dir = workspace_dir / "media"
    if not media_dir.is_dir():
        return []
    media_root = media_dir.resolve()
    candidates: list[tuple[str, Path, SkillInfo]] = []
    for skill_md in sorted(media_dir.rglob("SKILL.md")):
        skill_dir = skill_md.parent
        if not skill_dir.resolve().is_relative_to(media_root):
            continue
        skill = _read_skill_from_dir(skill_dir, UPLOADED_MEDIA_SKILL_SOURCE)
        if skill is None:
            continue
        name = _normalize_skill_dir_name(_resolve_skill_name(skill_dir))
        candidates.append((name, skill_dir, skill))
    return candidates


def _find_uploaded_skill(workspace_dir: Path, skill_name: str) -> tuple[str, Path] | None:
    target = _normalize_skill_dir_name(skill_name)
    for name, skill_dir, _skill in _uploaded_skill_candidates(workspace_dir):
...
def _install_uploaded_skill(
    ...
    *,
    ...
    candidate = _find_uploaded_skill(workspace_dir, skill_name)
    if candidate is None:
        return None
    resolved_name, source_dir = candidate
    target_dir = get_workspace_skills_dir(workspace_dir) / resolved_name
    if target_dir.exists() and not overwrite:
        return {"success": False, "name": resolved_name, "reason": "conflict"}

    _copy_skill_dir(source_dir, target_dir)
    reconcile_workspace_manifest(workspace_dir)
    result = SkillService(workspace_dir).enable_skill(resolved_name)
    ...
    return {"success": True, "name": resolved_name, "enabled": True}
...
@router.post("/uploaded/install")
async def install_uploaded_workspace_skill(
    request: Request,
    body: InstallUploadedSkillRequest,
) -> dict[str, Any]:
    workspace_dir = await _request_workspace_dir(request)
    try:
        result = _install_uploaded_skill(
            workspace_dir,
            body.skill_name,
    ...
    except (ValueError, AppBaseException, SkillsError) as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    if result is None:
        raise HTTPException(status_code=404, detail="Uploaded skill not found")
    if not result.get("success", False):
        status_code = 409 if result.get("reason") == "conflict" else 400
