...
from ...tenancy.cron_dispatch import TenantCronDispatchChannelManager
from ...config.config import load_agent_config

logger = logging.getLogger(__name__)


def _get_job_repo(ws: "Workspace"):
    """获取 job repo：优先 StorageManager，否则 JSON。"""
    from ..crons.repo.json_repo import JsonJobRepository
    from ...enterprise.storage.manager import get_storage_manager_from_app

    storage = get_storage_manager_from_app(ws._app) if ws._app else None
    if storage is not None:
        return storage.get_job_repository(ws.agent_id, str(ws.workspace_dir))
    return JsonJobRepository(str(ws.workspace_dir / "jobs.json"))
...
class Workspace:
    ...
    def __init__(
        ...
        # Non-service state
        self._config = agent_config  # Loaded before start()
        self._started = False
    ...
    def set_manager(self, manager) -> None:
        ...
        self._app = getattr(manager, "_app", None)
        ...
        sm.register(
            ServiceDescriptor(
                ...
                init_args=lambda ws: {  # pylint: disable=protected-access
                    "repo": _get_job_repo(ws),
                    "runner": ws._service_manager.services["runner"],
                    "channel_manager": TenantCronDispatchChannelManager(ws),
    ...
    async def start(self):
        ...
        try:
            ...
            if self._config is None:
                ...
                logger.debug(
                    f"Using pre-loaded config for agent: {self.agent_id}",
                )
