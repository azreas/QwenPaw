...
from ..config.agent_config_file import load_agent_config_from_workspace
...
class AgentConfigWatcher:
    ...
    def _get_agent_config(self):
        """返回 agent 配置，优先使用预加载配置。"""
        if self._preloaded_agent_config is not None:
            return self._preloaded_agent_config
    ...
    async def _check(self) -> None:
        ...
        try:
            agent_config = (
                ...
                or load_agent_config(self._agent_id)
            )
            if self._preloaded_agent_config is not None:
                updated = load_agent_config_from_workspace(
                    self._workspace_dir,
                )
