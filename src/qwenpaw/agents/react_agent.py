...
class QwenPawAgent(ToolGuardMixin, ReActAgent):
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        self._tool_policy_patch = tool_policy_patch
        ...
        # P3-2: 注册 MCP / built-in tool 调用追踪中间件
        self._mcp_tool_traces: list = []
        self._builtin_tool_traces: list = []
        from .mcp_tracer import (
            create_builtin_tool_tracing_middleware,
            create_mcp_tracing_middleware,
        )

        self.toolkit.register_middleware(
            create_mcp_tracing_middleware(self.toolkit, self),
        )
        self.toolkit.register_middleware(
            create_builtin_tool_tracing_middleware(self.toolkit, self),
        )
    ...
    @staticmethod
    def _is_tool_enabled_by_runtime_policy(
        tool_name: str, patch: Any
    ) -> bool:
        """检查工具是否被运行时策略允许。

        patch 为 None 或 disable_tools 中不包含该工具时返回 True。
        """
        if patch is None:
            return True
        return tool_name not in patch.disable_tools
    ...
    def _create_toolkit(
        ...
        for tool_name, tool_func in tool_functions.items():
            ...
            # 运行时策略检查（disable_tools）
            if not QwenPawAgent._is_tool_enabled_by_runtime_policy(
                tool_name, self._tool_policy_patch
            ):
                logger.info(
                    "Tool disabled by runtime policy: %s",
                    tool_name,
                )
                continue
