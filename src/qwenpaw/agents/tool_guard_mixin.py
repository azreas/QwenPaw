...
def _set_tool_guard_ctx(mixin_self: Any) -> Any:
    """从 agent 的 _request_context dict 构建 RequestContext 并设置 ContextVar。"""
    try:
        ...
        ctx = getattr(mixin_self, "_request_context", None) or {}
        enterprise_ctx = RequestContext(
            ...
            roles=tuple(ctx.get("roles", ())),
            actor=RequestActor(
                actor_id=ctx.get("user_id", ""),
                actor_type=ctx.get("actor_type", "channel_user"),
            ),
        )
        return set_current_request_context(enterprise_ctx)
    except Exception:
        return None


def _clear_tool_guard_ctx(token: Any) -> None:
    """清理 ContextVar。"""
    try:
        from qwenpaw.enterprise.context import clear_current_request_context
...
def _set_tool_policy_patch_ctx(mixin_self: Any) -> Any:
    """将 agent 的 _tool_policy_patch 设置到 ContextVar。"""
    try:
        from qwenpaw.enterprise.context import (
            set_current_tool_policy_patch,
        )

        patch = getattr(mixin_self, "_tool_policy_patch", None)
        return set_current_tool_policy_patch(patch)
    except Exception:
        return None


def _clear_tool_policy_patch_ctx(token: Any) -> None:
    """清理 tool policy patch ContextVar。"""
    try:
        from qwenpaw.enterprise.context import (
            clear_current_tool_policy_patch,
        )

        clear_current_tool_policy_patch(token)
    except Exception:
        pass
...
def _create_quota_guard_result(
    tool_name: str,
    tool_input: dict[str, Any],
    *,
    reason: str,
    ...
    finding = GuardFinding(
        ...
        rule_id="quota_policy_deny",
        category=GuardThreatCategory.RESOURCE_ABUSE,
        severity=GuardSeverity.CRITICAL,
        title="Denied by quota policy",
        description=reason,
        tool_name=tool_name,
        param_name=None,
        matched_value=None,
        ...
        snippet=None,
        remediation="Reduce request rate or increase quota",
        guardian="QuotaPolicy",
...
class ToolGuardMixin:
    ...
    async def _acting(self, tool_call) -> dict | None:  # noqa: C901
        ...
        # 设置 enterprise request context ContextVar 供 tool_guard 审计使用
        _token = _set_tool_guard_ctx(self)

        # 设置 per-request tool policy 到 ContextVar
        _policy_token = _set_tool_policy_patch_ctx(self)
        ...
        try:
            ...
            if _policy_token is not None:
                _clear_tool_policy_patch_ctx(_policy_token)
            if _token is not None:
                _clear_tool_guard_ctx(_token)
    ...
    async def _decide_guard_action(
        ...
        # 检查 per-request deny（运行时策略），不污染全局 _denied_tools
        # 通过 engine.guard() 生成带 finding 的 ToolGuardResult，
        # 确保拒绝行为也能被 P0-3 审计捕获
        from qwenpaw.enterprise.context import (
        ...
        if req_patch is not None and tool_name in req_patch.deny_tools:
            logger.warning(
                "Tool guard: tool '%s' denied by runtime policy",
                tool_name,
            )
            # 调用 engine.guard() 以生成带审计 finding 的结果
            runtime_deny_result = engine.guard(
            ...
            return _GuardAction(
                ...
                tool_name,
                tool_input,
                guard_result=runtime_deny_result,
            )

        # Quota check: deny if tool call exceeds quota limit
        try:
            runtime = get_enterprise_runtime()
            quota = getattr(runtime, "quota", None) if runtime is not None else None
            quota_decision = await consume_tool_call(
                quota,
                get_current_request_context(),
                tool_name,
            )
            if quota_decision is not None and not quota_decision.allowed:
                logger.warning(
                    "Tool guard: tool '%s' denied by quota policy",
                    tool_name,
                )
                quota_result = _create_quota_guard_result(
                    tool_name,
                    tool_input,
                    reason=quota_decision.reason or "quota exceeded",
                )
                return _GuardAction(
                    "auto_denied",
                    tool_name,
            ...
            logger.debug("Tool quota check skipped", exc_info=True)
