...
本仓库以开源 QwenPaw 为基础，扩展了企业级多租户和运营管理能力：除个人 AI 助手能力外，还新增了 WebChat / 企微 Bot 双入口、`wx_*` 动态租户工作区、企业运行时、RBAC、审计追踪、租户级 Skills/MCP 配置和 Console 运营后台。

本地试用可以保持 Console 认证关闭；企业受保护环境应显式开启 `QWENPAW_AUTH_ENABLED`，并配置管理员账号、WebChat Session 密钥和所需存储后端。企业级扩展的整体说明和 403 排查方式见 [企业级扩展说明](docs/enterprise/README_zh.md)，该说明只引用公开仓库内的常规文档和代码路径。
...
> - **我想了解当前企业级扩展**：[当前仓库情况](#当前仓库情况) → [企业级扩展说明](docs/enterprise/README_zh.md)
...
| [企业级扩展说明](docs/enterprise/README_zh.md)              | 当前 fork 的企业级能力、使用入口、运行配置和使用边界 |
